...
DeepConvNet & 0.4716 & 0.4707 & 0.4716 & 0.4677 \\
\hline
\end{tabular}
\end{table*}
...
El modelo DeepConvNet alcanzó un accuracy de 47.16\%, una precisión de 47.07\%, un recall de 47.16\% y un F1-score de 0.4677 sobre el conjunto de prueba. Estos resultados son inferiores a los obtenidos por el modelo BoF--SVM, sugiriendo que, en este contexto específico y con esta configuración, el aprendizaje profundo no superó el enfoque basado en características manuales. El desempeño cercano al azar (50\%) puede atribuirse a factores como el tamaño limitado del conjunto de datos (880 ensayos distribuidos en 20 sujetos), la complejidad de la arquitectura en relación con la cantidad de datos disponibles, o la necesidad de ajustes más finos en la arquitectura y los hiperparámetros de entrenamiento.

\subsection{Análisis Comparativo}

La comparación directa entre ambos modelos revela que el enfoque BoF--SVM superó al DeepConvNet en todas las métricas evaluadas, con diferencias de aproximadamente 5.7 puntos porcentuales en accuracy y 7.7 puntos porcentuales en F1-score (Fig.~\ref{fig:metrics_comparison}). Esta diferencia sugiere que, en este escenario específico, la extracción explícita de características mediante transformadas wavelet, combinada con el modelo BoF, fue más efectiva que el aprendizaje automático de características mediante convoluciones. Sin embargo, ambos modelos presentan un desempeño que, aunque superior al azar, está por debajo de los valores reportados en la literatura (Tabla~\ref{tab:sota}), lo cual puede atribuirse a la complejidad del problema de generalización inter-sujeto, la variabilidad inherente de las señales EEG y las características específicas de este conjunto de datos.
...
Los resultados obtenidos en este estudio proporcionan insights valiosos sobre las ventajas y limitaciones de los enfoques basados en características manuales versus aprendizaje profundo para la clasificación de señales EEG de imaginación motora. El mejor desempeño del modelo BoF--SVM en comparación con DeepConvNet contrasta con tendencias observadas en otros dominios donde las redes profundas suelen superar a métodos clásicos. Esta discrepancia puede explicarse por varios factores metodológicos y del dominio.

\subsection{Interpretación de Resultados}

El desempeño superior del modelo BoF--SVM puede atribuirse a la explícita explotación de conocimiento del dominio a través de las transformadas wavelet, las cuales están diseñadas específicamente para capturar patrones tiempo--frecuencia característicos de la actividad sensorimotora. Las bandas $\mu$ y $\beta$ están bien establecidas en la literatura como marcadores neurofisiológicos de la imaginación motora~\cite{ref1, ref2}, y su extracción mediante CWT y DWT proporciona una representación rica y discriminativa. Por el contrario, el modelo DeepConvNet requiere aprender estas relaciones desde cero, lo cual puede ser desafiante con conjuntos de datos de tamaño moderado como el empleado en este estudio (880 ensayos). La necesidad de generalización inter-sujeto, evaluada mediante GroupKFold, añade una capa adicional de complejidad que puede favorecer enfoques con menor capacidad expresiva pero mejor regularización implícita.

El rendimiento cercano al azar de ambos modelos, aunque superior a 50\%, sugiere que el problema de clasificación MI-EEG presenta desafíos significativos que van más allá de la elección del algoritmo. La variabilidad inter e intra-sujeto, la baja relación señal--ruido inherente a las señales EEG, y la necesidad de calibración individual son factores que impactan consistentemente el desempeño en tareas BCI. Estos resultados son coherentes con estudios previos que reportan que, aunque los métodos de aprendizaje automático pueden alcanzar altos niveles de precisión en condiciones controladas o con calibración por sujeto, su generalización a nuevos individuos sigue siendo un desafío abierto~\cite{ref2, ref4}.

\subsection{Errores y Limitaciones}

El análisis de errores revela que ambos modelos presentan dificultades similares en la discriminación entre clases, con matrices de confusión que muestran un número equilibrado de falsos positivos y falsos negativos. En el caso del modelo BoF--SVM, la ligera asimetría hacia un mayor recall sugiere que el modelo es más conservador en rechazar casos positivos, lo cual puede ser deseable en aplicaciones BCI donde la detección de intentos de movimiento es crítica. Sin embargo, el costo es una precisión ligeramente reducida.
...
En respuesta a la pregunta de investigación, el enfoque BoF--SVM demostró ser más efectivo que el modelo CNN para la clasificación de señales EEG de imaginación motora en este contexto específico, alcanzando un accuracy de 52.84\% y un F1-score de 0.5451, comparado con 47.16\% y 0.4677 respectivamente para DeepConvNet. Esta diferencia, aunque modesta en términos absolutos, es estadísticamente relevante y sugiere que la explotación explícita de conocimiento del dominio a través de transformadas wavelet proporciona ventajas cuando se trabaja con conjuntos de datos de tamaño moderado y bajo condiciones de validación inter-sujeto.

Respecto a los objetivos específicos: (1) el modelo BoF--SVM fue implementado exitosamente, aunque no alcanzó el objetivo de 80\% de accuracy inicialmente establecido, obteniendo 52.84\%; (2) el modelo CNN también fue implementado, pero tampoco superó el objetivo de 85\% de accuracy, alcanzando 47.16\%; y (3) el análisis comparativo utilizando métricas múltiples (accuracy, precision, recall, F1-score) identificó que BoF--SVM presenta mayor robustez y consistencia, con mejores valores en todas las métricas evaluadas.

Estos resultados reflejan la complejidad inherente del problema de clasificación MI-EEG y destacan la importancia de considerar el contexto específico (tamaño del dataset, necesidad de generalización, recursos computacionales) al seleccionar el enfoque metodológico. Mientras que el aprendizaje profundo ha demostrado ser altamente efectivo en otros dominios, su aplicación exitosa a señales EEG requiere adaptaciones cuidadosas, consideración de las limitaciones de datos, y posiblemente la incorporación de conocimiento del dominio en la arquitectura o el preprocesamiento.
